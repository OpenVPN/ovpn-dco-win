...
IROUTE_LOAD=10.90.0.9     # belongs to no one, so the load always reaches the trie
IROUTE_NET=10.90.0.0/24   # must match the ccd iroute the server hands out
...
# The client configs use log-append, and the results directory is reused between runs,
# so clear what this run is about to write. Otherwise everything counted by grepping
# these logs -- flood connects, client restarts, iperf3 results -- carries the previous
# run's numbers as well as this one's.
rm -f "$R"/c*.log "$R"/f*.log "$R"/*.cli.log "$R"/*.tally "$R"/*.vpnip 2>/dev/null
NT=$((PAIRS * 2))
...
vpndev() { sudo ip netns exec "$1" sh -c 'ls /sys/class/net | grep "^tun" | head -1'; }
...
# Every client is given the same iroute in ccd/DEFAULT, because they all share one
# certificate and the server cannot tell them apart. So whichever peer connected last
# owns the prefix, and no one can say which that is without asking.
#
# Give the probe address to all of them and the question goes away: the server pings it,
# and whoever owns it answers. A reply means the driver resolved the prefix through the
# trie and delivered it to a peer, which is the whole point of the probe.
check_iroute() {
    local i ns dev
    [ -z "$DUT_SSH" ] && { echo "  iroute: skipped, no --dut given"; return; }
    for i in $(seq 1 $NT); do
        ns=c$i
        dev=$(vpndev "$ns") || continue
        [ -z "$dev" ] && continue
        sudo ip netns exec "$ns" ip addr add "$IROUTE_PROBE/32" dev "$dev" 2>/dev/null
    done
    if ssh "$DUT_SSH" "ping -n 4 -w 1000 $IROUTE_PROBE" >/dev/null 2>&1; then
        echo "  iroute: server -> $IROUTE_PROBE was answered, so the trie delivered it"
        iroute_ok=1
    else
        echo "  iroute: server -> $IROUTE_PROBE reached no peer"
    fi
}

# The driver checks the source of everything it decrypts against the peer that sent it,
# and falls back to the iroute trie when the source is not that peer's own address.
# Nothing else in the run reaches that fallback, because a client only ever sends from
# its tunnel address.
#
# Only the peer that owns the prefix can pass: for anyone else the trie resolves to a
# different peer and the driver drops it, which is the filter working. So try each in
# turn — the one that gets an answer is the owner, and that is how we learn which it is.
check_rpf() {
    local i ns
    for i in $(seq 1 $NT); do
        ns=c$i
        if sudo ip netns exec "$ns" ping -c 2 -W 2 -q -I "$IROUTE_PROBE" "$SERVER_TUN" >/dev/null 2>&1; then
            echo "  rpf: $ns owns $IROUTE_NET, and the server accepted its packet sourced from $IROUTE_PROBE"
            rpf_ok=1; IROUTE_OWNER=$ns
            return
        fi
    done
    echo "  rpf: no peer could send from $IROUTE_PROBE, so none of them owns $IROUTE_NET"
}
...
# Both probes must run before the swarm and the flood start, and they do. Every peer
# claims the prefix on connect, so the owner is whoever connected last: while only the
# traffic clients are up it is one of them, and they can answer. Once the flood is
# running the owner is usually a dev null client with no interface at all, which would
# resolve and deliver correctly and still never reply — a pass the probe cannot see.
iroute_ok=0; rpf_ok=0; IROUTE_OWNER=""   # the owner is reported, not needed by anything
[ "$NT" -ge 2 ] && check_iroute
[ "$NT" -ge 2 ] && check_rpf

# Ownership of the prefix changes thousands of times a run while almost nothing looks it
# up: the traffic pairs address each other directly, which the peer table answers before
# the trie is consulted. So send to the subnet steadily, from every client at once, to
# keep IPTrie::Find on the datapath while the inserts hand the prefix along.
#
# Aimed at an address no client holds, so the packet always has to be resolved through
# the trie rather than answered locally by whoever happens to own the probe address.
# Nothing replies, which is expected: the lookup is the point, not the round trip.
#
# The route has to be added by hand. OpenVPN strips a pushed route from any client that
# owns the matching iroute, and ccd/DEFAULT gives it to every one of them, so without
# this the packets leave through the namespace default route and never reach the driver.
trie_senders=""
if [ "$NT" -ge 2 ]; then
    for i in $(seq 1 $NT); do
        ns=c$i
        dev=$(vpndev "$ns") || continue
        [ -z "$dev" ] && continue
        sudo ip netns exec "$ns" ip route replace "$IROUTE_NET" dev "$dev" 2>/dev/null
        sudo ip netns exec "$ns" sh -c "while :; do ping -c 100 -i 0.01 -W 1 -q $IROUTE_LOAD; done" \
            >/dev/null 2>&1 &
        trie_senders="$trie_senders $!"
        disown %% 2>/dev/null || true
    done
    echo "  trie: $(echo $trie_senders | wc -w) clients -> $IROUTE_LOAD at 100/s each"
fi
...
printf '%6s  %8s  %8s  %8s  %8s  %-18s %s\n' \
    elapsed 'sw conn' 'sw t/o' 'flood' traffic per-pair 'total Mbit/s'
while [ "$(date +%s)" -lt $END ]; do
    sleep 30
    ...
    # a reconnect builds a new tun and loses the route the trie senders need
    for i in $(seq 1 $NT); do
        ns=c$i; dev=$(vpndev "$ns")
        [ -n "$dev" ] && sudo ip netns exec "$ns" ip route replace "$IROUTE_NET" dev "$dev" 2>/dev/null
    done
    up=0
    ...
    # The swarm is the small half of the churn; the flood is two orders of magnitude
    # larger, and a line without it reads as though almost nothing is connecting.
    flood_now=$(cat "$R"/f*.log 2>/dev/null | grep -c "Initialization Sequence Completed")
    printf '%5ds  %8s  %8s  %8s  %6s/%d  %-18s %s\n' \
        "$(( $(date +%s) - START ))" "$(tally 1)" "$(tally 2)" "${flood_now:-0}" "$up" "$NT" "$tp" "$total"
done
...
wait $swarm_pids
for p in $trie_senders; do kill "$p" 2>/dev/null; done
sudo pkill -f "ping -c 100 -i 0.01" 2>/dev/null
if [ -n "$flood_pid" ]; then
