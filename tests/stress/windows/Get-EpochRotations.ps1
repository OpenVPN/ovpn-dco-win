...
$result = [pscustomobject]@{
    ...
    # A key was found and the tag did not verify. It dominates the decrypt failures and
    # had no counter, so most of DecryptErrors was unexplained.
    AuthFailed      = ([regex]::Matches($text, 'Packet failed authentication')).Count
