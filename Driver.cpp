...
OvpnEvtDeviceAdd(WDFDRIVER wdfDriver, PWDFDEVICE_INIT deviceInit) {
    ...
    // security descriptor for the device object, see below
    DECLARE_CONST_UNICODE_STRING(deviceSddl, L"D:P(A;;GA;;;SY)(A;;GA;;;BA)(A;;GRGWGX;;;WD)(A;;GRGX;;;RC)");
    ...
    // Without an explicit security descriptor the framework creates the device with
    // IoCreateDevice() and it inherits the security of the PDO, which on some systems
    // is admin-only and then denies CreateFile() from a non-elevated openvpn.exe.
    //
    // This is what the device object gets by default on a system where opening it
    // works, so the only thing that changes is that the descriptor is now ours
    // instead of inherited. The SDDL_DEVOBJ_* constants are deliberately avoided:
    // the closest one also drops DELETE/WRITE_DAC/WRITE_OWNER for Administrators.
    //
    // A security descriptor can only be attached to a named device object and
    // NetAdapterCx does not name the FDO, hence FILE_AUTOGENERATED_DEVICE_NAME.
    WdfDeviceInitSetCharacteristics(deviceInit, FILE_AUTOGENERATED_DEVICE_NAME, TRUE);
    GOTO_IF_NOT_NT_SUCCESS(done, status, WdfDeviceInitAssignSDDLString(deviceInit, &deviceSddl));
